  ...
  describe('resolveLabel()', () => {
    ...
    it('returns a combined org/email label from the profile endpoint', async () => {
      vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
        profileResponse({ orgName: 'Acme', email: 'user@example.com' }),
      );
      const source = new ClaudeCodeSource(new InMemoryBackend());
    ...
    it('unwraps claudeAiOauth envelope to extract tokens', async () => {
      const fetchSpy = vi
        ...
        .mockResolvedValueOnce(profileResponse({ orgName: 'EnvOrg', email: 'env@example.com' }));
      const source = new ClaudeCodeSource(new InMemoryBackend());
    ...
    it('uses cached profile from a prior read() call', async () => {
      const fetchSpy = vi
        .spyOn(globalThis, 'fetch')
        .mockResolvedValue(profileResponse({ orgName: 'CachedOrg', email: 'cached@example.com' }));
