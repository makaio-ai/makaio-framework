...
// ---------------------------------------------------------------------------
// Keychain mock (hoisted so vi.mock factory can reference it)
// ---------------------------------------------------------------------------

const mockKeychain = vi.hoisted(() => {
  const store = new Map<string, string>();
  return {
    keychainRead: vi.fn(async (service: string, account?: string) => {
      return store.get(`${service}:${account ?? ''}`) ?? null;
    }),
    keychainWrite: vi.fn(async (service: string, account: string, value: string) => {
...
vi.mock('../utils/security-cli.js', () => ({
  keychainRead: mockKeychain.keychainRead,
  keychainWrite: mockKeychain.keychainWrite,
}));
...
describe.skipIf(process.platform !== 'darwin')('DarwinAccountStore', () => {
  ...
  describe('empty store', () => {
    ...
    it('does not touch the keychain when the store file is absent', async () => {
      ...
      expect(mockKeychain.keychainRead).not.toHaveBeenCalled();
      expect(mockKeychain.keychainWrite).not.toHaveBeenCalled();
    });
  });
