/**
 * esbuild plugin that embeds Drizzle migration SQL at build time.
 *
 ...
 * This eliminates filesystem access for migrations in the bundled Electron
 * app — no temp dirs, no path resolution, no asar gotchas.
 *
 * @example
 * ```ts
 * await build({
...
/**
 * Create an esbuild plugin that embeds Drizzle migrations at build time.
 *
...
export function embeddedMigrationsPlugin(
  ...
  return {
    ...
    setup(build) {
      ...
      // onResolve fires BEFORE esbuild's default resolution, so this
      ...
      build.onLoad({ filter: /.*/, namespace: 'embedded-migrations' }, () => {
        ...
        const watchFiles = embeddedMigrations.sources.flatMap(({ migrationsDir, migrationSourceId }) => {
          const migrations = embeddedMigrations.migrationsBySourceId.get(migrationSourceId) ?? [];
          return [
            `${migrationsDir}/meta/_journal.json`,
            ...migrations.map((migration) => `${migrationsDir}/${migration.tag}.sql`),
          ];
        });
