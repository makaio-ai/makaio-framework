...
function createWindowManagerStub(windows: WindowManagerState[] = []) {
  return {
    listWindows: vi.fn<() => WindowManagerState[]>(() => windows),
    focusWindow: vi.fn<(windowId: number) => boolean>(() => true),
  };
}
