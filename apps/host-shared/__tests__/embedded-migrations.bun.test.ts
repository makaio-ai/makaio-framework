...
/**
 * Import a generated JS module source string.
 *
 * Bun enforces a URL length limit that prevents importing large modules via
 * `data:` URLs. We work around this by writing the source to a temp file and
 * importing from the file path instead.
 * @param source - Generated JavaScript module source.
 * @returns The imported module.
 */
async function importGeneratedSource(source: string): Promise<Record<string, unknown>> {
  const tmpFile = path.join(os.tmpdir(), `makaio-gen-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`);
...
describe('embedded build migrations', () => {
  it('renders a bundled migration reader that resolves the requested migration directory', async () => {
    ...
    const mod = await importGeneratedSource(source);
    const readMigrations = mod.readMigrations as (...args: unknown[]) => unknown;

    expect(readMigrations()).toEqual([
    ...
    expect(readMigrations(trustedDevicesDir)).toEqual([
      expect.objectContaining({
        tag: '0000_trusted_devices',
        folderMillis: 2,
        sql: ['CREATE TABLE `trusted_devices` (`id` text PRIMARY KEY NOT NULL);'],
    ...
    expect(readMigrations(path.join(trustedDevicesDir, '..', path.basename(trustedDevicesDir)))).toEqual([
      expect.objectContaining({
        tag: '0000_trusted_devices',
        folderMillis: 2,
      }),
    ]);
    expect(readMigrations(buildMigrationSourceId(workspaceRoot, trustedDevicesDir))).toEqual([
      expect.objectContaining({
        tag: '0000_trusted_devices',
        folderMillis: 2,
      }),
    ]);
    expect(() => readMigrations(path.join(workspaceRoot, 'missing', 'drizzle'))).toThrow(/no embedded migrations/i);
  });
  ...
  it('rejects migration directories outside the workspace root even when path.relative returns an absolute path', () => {
    ...
    const relativeSpy = spyOn(path, 'relative').mockImplementation((from: string, to: string) => {
    ...
    const isAbsoluteSpy = spyOn(path, 'isAbsolute').mockImplementation((candidate: string) => {
