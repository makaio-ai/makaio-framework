...
/**
 * Configures TypeDoc source links for the standalone framework repository.
...
/**
 * Rewrites generated source links to the public standalone framework repository.
...
export function normalizeFrameworkSourceLinks(content: string): string {
  return content.replaceAll(
    /\[([^\]]+)\]\(https:\/\/github\.com\/makaio-ai\/(?:makaio|makaio-framework)\/blob\/[^/)]+\/framework\/([^)#\s]+)(#[^)\s]+)?\)/gu,
