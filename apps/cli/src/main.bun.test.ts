...
const busClientMocks = {
  connectBusClient: mock<(...args: unknown[]) => Promise<unknown>>(),
  isAuthConnectionError: mock<(error: unknown) => boolean>(),
  probeHealth: mock<(url: string) => Promise<unknown>>(),
  resolveClientAuth: mock<(...args: unknown[]) => unknown>(),
  resolveBusUrl: mock<(...args: unknown[]) => string>().mockReturnValue('ws://127.0.0.1:6252/bus'),
};
const serveMocks = {
  serve: mock<(options: ServeOptions) => Promise<void>>(),
};
const appLaunchMocks = {
  launchAppAndWaitForBus: mock<(url: string) => Promise<unknown>>().mockResolvedValue({
    health: null,
    launched: false,
  }),
};
...
mock.module('./serve.js', () => serveMocks);
mock.module('./app-launch.js', () => appLaunchMocks);
...
describe('main — remote manifest behavior', () => {
  beforeEach(() => {
    ...
    mock.clearAllMocks();
    busClientMocks.isAuthConnectionError.mockImplementation((error: unknown) => {
      const message = error instanceof Error ? error.message : String(error);
      return /\b(401|403|auth|unauthori[sz]ed|forbidden|credential|secret)\b/i.test(message);
    });
    busClientMocks.resolveBusUrl.mockReturnValue('ws://127.0.0.1:6252/bus');
  ...
  it('passes config-derived discovery, launcher command, and package defaults to serve', async () => {
    ...
    try {
      ...
      const serveCall = serveMocks.serve.mock.calls[0];
      if (serveCall === undefined) {
        throw new Error('Expected serve to be called');
      }
      const [serveOptions] = serveCall as [ServeOptions];
  ...
  it('uses env-selected config to discover local source-tree commands', async () => {
    ...
    try {
      ...
      expect(busClientMocks.probeHealth).toHaveBeenCalledTimes(1);
    } finally {
  ...
  it('fails clearly when a remote-only command only supports interactive execution', async () => {
    ...
    request.mockImplementation(async (subject: unknown) => {
  ...
  it('surfaces an auth-specific connection error when the auth handshake fails for a local interactive command', async () => {
    ...
    localDiscovery.discover = mock().mockResolvedValue([
      {
        descriptor: {
          name: 'local-url-test',
    ...
    busClientMocks.resolveClientAuth.mockReturnValue({ token: 'secret' });
    busClientMocks.connectBusClient.mockRejectedValue(new Error('auth handshake failed'));
    ...
    try {
      ...
      expect(busClientMocks.probeHealth).toHaveBeenCalledTimes(1);
      expect(busClientMocks.connectBusClient).toHaveBeenCalledTimes(1);
    } finally {
      ttyFixture.restore();
  ...
  it('reports a background launch timeout instead of the generic server-start hint', async () => {
    ...
    busClientMocks.probeHealth.mockResolvedValue(null);
    appLaunchMocks.launchAppAndWaitForBus.mockResolvedValue({ health: null, launched: true });
  ...
  it('keeps generic connection wording when auth is enabled but the failure is not auth-related', async () => {
    ...
    busClientMocks.probeHealth.mockResolvedValue({ auth: true });
    busClientMocks.resolveClientAuth.mockReturnValue({ token: 'secret' });
    busClientMocks.connectBusClient.mockRejectedValue(new Error('socket closed'));
