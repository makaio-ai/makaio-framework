...
const runtimeNodeMocks = vi.hoisted(() => ({
  findWorkspaceRootInfo:
    ...
    >(),
  createMakaioConfigDiscovery: vi.fn((config: unknown) => ({ config })),
  loadMakaioConfig: vi.fn(async () => ({
  ...
  resolveMakaioHome: vi.fn(() => '/tmp/.makaio'),
  WorkspaceRootNotFoundError: class WorkspaceRootNotFoundError extends Error {
...
describe('discoverDevWorkspacePackages', () => {
  ...
  it('returns undefined when findWorkspaceRootInfo throws WorkspaceRootNotFoundError', async () => {
    runtimeNodeMocks.findWorkspaceRootInfo.mockImplementation((startDir: string) => {
      throw new runtimeNodeMocks.WorkspaceRootNotFoundError(startDir);
    });
...
describe('applyDevWorkspacePackages', () => {
  ...
  it('preserves other top-level serve config fields', () => {
    const resolver = vi.fn<(peerId: string) => Promise<CryptoKey | null>>();
