...
describe('ArtifactViewBuilderRegistry', () => {
  ...
  describe('open string kinds', () => {
    it('accepts arbitrary kind strings not known to framework source', () => {
      const registry = new ArtifactViewBuilderRegistry();
      const builder = makeBuilder('product-custom-widget', '3.2.1', 1);

      expect(() => {
        registry.replaceBuildersForOwner('ext-a', [builder]);
      }).not.toThrow();

      expect(registry.getBuilder('product-custom-widget', '3.2.1')).toBeDefined();
    });

    it('accepts arbitrary schema version strings', () => {
      const registry = new ArtifactViewBuilderRegistry();
      const builder = makeBuilder('review-report', '2024-07-13', 1);

      expect(() => {
        registry.replaceBuildersForOwner('ext-a', [builder]);
      }).not.toThrow();

      expect(registry.getBuilder('review-report', '2024-07-13')).toBeDefined();
    });

    it('does not conflate kind/schemaVersion pairs that concatenate identically', () => {
      const registry = new ArtifactViewBuilderRegistry();
      // Both pairs would collapse to 'a@b@c' under naive delimiter
      // concatenation; the tuple encoding must keep them distinct.
      registry.replaceBuildersForOwner('ext-a', [makeBuilder('a@b', 'c', 1), makeBuilder('a', 'b@c', 2)]);

      expect(registry.getBuilder('a@b', 'c')!.version).toBe(1);
      expect(registry.getBuilder('a', 'b@c')!.version).toBe(2);
    });
  });
