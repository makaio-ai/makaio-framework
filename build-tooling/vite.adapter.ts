...
import { frameworkImportRewritePlugin } from './framework-import-rewrite-plugin.js';
import {
...
function createTargetConfigForAdapter(target: BuildTarget, options: AdapterConfigOptions): UserConfig {
  ...
  const config: UserConfig = {
    root: packageRoot,
    plugins: [frameworkImportRewritePlugin(), createDtsPlugin(outDir, packageRoot), ...plugins],
    build: {
      outDir,
      emptyOutDir: true,
      sourcemap: true,
      ...
      rollupOptions: {
        external: (id) => {
          ...
          // Framework umbrella subpath imports are always external — they are
          // emitted by frameworkImportRewritePlugin and must not be rebundled.
          if (id === '@makaio/framework' || id.startsWith('@makaio/framework/')) return true;
