...
describe('extensionExternals plugin', () => {
  ...
  describe('framework-owned package externalization', () => {
    it('externalizes @makaio/bus-core and rewrites specifier to umbrella subpath', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      expect(resolve('@makaio/bus-core')).toEqual({ id: '@makaio/framework/bus', external: true });
    });

    it('externalizes @makaio/bus-core subpath imports and rewrites specifier', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      expect(resolve('@makaio/bus-core/types')).toEqual({
        id: '@makaio/framework/bus/types',
        external: true,
      });
    });

    it('externalizes deep @makaio/bus-core subpath imports and rewrites specifier', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      expect(resolve('@makaio/bus-core/internal/registry')).toEqual({
        id: '@makaio/framework/bus/internal/registry',
        external: true,
      });
    });

    it('externalizes @makaio/contracts and rewrites specifier', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      expect(resolve('@makaio/contracts')).toEqual({ id: '@makaio/framework/contracts', external: true });
    });

    it('externalizes @makaio/contracts subpath imports and rewrites specifier', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      expect(resolve('@makaio/contracts/extension')).toEqual({
        id: '@makaio/framework/contracts/extension',
        external: true,
      });
    });
  });
  ...
  describe('non-externalized packages', () => {
    it('rewrites framework-owned @makaio/* packages rather than bundling them', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
      // @makaio/core is framework-owned; it is externalized with specifier rewrite
      expect(resolve('@makaio/core')).toEqual({ id: '@makaio/framework/core', external: true });
    });

    it('returns null for @makaio/framework umbrella subpath imports (not workspace packages)', () => {
      const resolve = getResolveIdHandler(getExternalsPlugin());
