...
/**
 * Build a fully-featured mock target transport with mock() spies on every
...
describe('LoopbackTransport', () => {
  ...
  describe('send()', () => {
    ...
    it('returns whatever the target send() resolves with', async () => {
      const target = makeFullTarget();
      (target.send as ReturnType<typeof mock>).mockResolvedValue({ output: 'delegated' });
      const loopback = new LoopbackTransport({ target, name: 'loopback' });

      const result = await loopback.send(makeEventMessage());

      expect(result as unknown).toEqual({ output: 'delegated' });
    });
  });
  ...
  describe('isReady()', () => {
    it('delegates to target isReady() and returns its value', () => {
      const target = makeFullTarget();
      (target.isReady as ReturnType<typeof mock>).mockReturnValue(false);
      const loopback = new LoopbackTransport({ target, name: 'loopback' });
