...
describe('resolveWorkspaceRoot', () => {
  ...
  it('resolves the root in the nested framework layout (3 levels deep)', () => {
  ...
  it('resolves the root in the nested framework layout (4 levels deep)', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'makaio-workspace-root-'));
    tempDirs.push(root);
  ...
  it('resolves the root in the standalone framework layout', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'makaio-workspace-root-'));
    tempDirs.push(root);
  ...
  it('throws when neither nested nor standalone layout is detected', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'makaio-workspace-root-'));
    tempDirs.push(root);
