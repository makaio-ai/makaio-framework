...
describe('ExtensionCoordinator contribution processor lifecycle', () => {
  it('calls processActivated on boot, processStopped on disable, and processActivated again on re-enable', async () => {
    ...
    const activated = mock<ContributionProcessor['processActivated']>().mockResolvedValue(undefined);
    const stopped = mock<NonNullable<ContributionProcessor['processStopped']>>().mockResolvedValue(undefined);
  ...
  it('rolls back earlier processors when a later processor throws during activation', async () => {
    ...
    const firstStopped = mock<NonNullable<ContributionProcessor['processStopped']>>().mockResolvedValue(undefined);
  ...
  it('transitions active entries to stopped during shutdown and does not stop contributions twice', async () => {
    ...
    const stopped = mock<NonNullable<ContributionProcessor['processStopped']>>(async () => {
