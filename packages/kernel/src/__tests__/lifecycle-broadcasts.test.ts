...
import type { MakaioExtension, NodeExtensionContext as ExtensionContext } from '@makaio/contracts';
...
describe('lifecycle broadcast subjects', () => {
  ...
  it('all handlers on coordinatorReady execute when two packages register', async () => {
    const handlerA = vi.fn((ctx) => ctx.setResult({}));
    const handlerB = vi.fn((ctx) => ctx.setResult({}));
