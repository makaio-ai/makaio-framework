...
function makeStrategyDeps(
  ...
  return {
    fetchText: mock().mockResolvedValue('1.0.0'),
    fetchJson: mock().mockResolvedValue({ sha256: CHECKSUM }),
    downloadFile: mock().mockImplementation(async (_url: string, dest: string) => {
      if (executeDelayMs > 0) {
        await new Promise<void>((resolve) => setTimeout(resolve, executeDelayMs));
      }
      return dest;
    }),
    computeChecksum: mock().mockResolvedValue(CHECKSUM),
    extractArchive: mock().mockResolvedValue(undefined),
    deleteFile: mock().mockResolvedValue(undefined),
    removeDirectory: mock().mockResolvedValue(undefined),
...
function createPostInstallFailureHarness(
  ...
  const onProgress: JobProgressCallback = mock();
  const onComplete: JobCompletionCallback = mock().mockResolvedValue(undefined);
...
function createCompletionSpy<TPayload extends object = ClientInstallCompleted>(
  ...
  const onCompleted: JobCompletedCallback = mock().mockImplementation(async (payload) => {
...
describe('ClientBinaryJobRunner', () => {
  ...
  it('removes the installed artifact when persistence rejects the completed install', async () => {
    ...
    const onProgress: JobProgressCallback = mock();
    const onComplete: JobCompletionCallback = mock().mockRejectedValue(new Error('storage write failed'));
    const { onCompleted, completed } = createCompletionSpy(completedPayloads);
  ...
  it('removes the target directory when strategy execution fails after staging files', async () => {
    ...
    const strategyDeps = {
      ...
      removeDirectory: mock().mockImplementation(async (dirPath: string) => {
        await fs.rm(dirPath, { recursive: true, force: true });
      }),
    } satisfies StrategyDependencies;
    const cleanupRunner = new ClientBinaryJobRunner(strategyDeps, {
  ...
  it('cancelAll suppresses onComplete and onCompleted for a job that is still in flight', async () => {
    ...
    const onComplete: JobCompletionCallback = mock().mockResolvedValue(undefined);
    const onCompleted: JobCompletedCallback = mock().mockResolvedValue(undefined);
    const onProgress: JobProgressCallback = mock();
  ...
  it('cancelAll from verifying progress suppresses the version-command exec, onComplete, and onCompleted', async () => {
    ...
    const execCalls = (strategyDeps.exec as ReturnType<typeof mock>).mock.calls as unknown[][];
  ...
  it('cleans the staged artifact and emits status error when the registered post-install handler throws', async () => {
    ...
    const throwingHandler = mock().mockRejectedValue(new Error('chmod failed: permission denied'));
    const {
  ...
  it('emits the activating stage before calling onComplete', async () => {
    ...
    const onComplete: JobCompletionCallback = mock().mockImplementation(async () => {
