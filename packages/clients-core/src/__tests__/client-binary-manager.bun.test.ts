...
function makeStrategyDeps(
  ...
  return {
    ...
    fetchText: mock().mockResolvedValue(''),
    fetchJson: mock().mockResolvedValue({}),
    downloadFile: mock().mockResolvedValue(''),
    computeChecksum: mock().mockResolvedValue(''),
    ...
    exec: mock().mockImplementation(async (cmd: string, args: string[], opts?: { cwd?: string }) => {
      if (executeDelayMs > 0) {
...
describe('ClientBinaryManager', () => {
  ...
  it('runs a registered post-install handler and emits the post-install stage', async () => {
    const postInstallHandler = mock().mockResolvedValue({ mode: '755' });
    await initManager(makeStrategyDeps(), {
  ...
  it('post-install runs before version verification', async () => {
    ...
    const postInstallHandler = mock().mockImplementation(async () => {
      callOrder.push('post-install');
      return { mode: '755' };
    });
    const strategyDeps = makeStrategyDeps();
    (
      strategyDeps.exec as Mock<(cmd: string, args: string[], opts?: { cwd?: string }) => Promise<string>>
    ).mockImplementation(async (cmd: string, args: string[], opts?: { cwd?: string }) => {
      if (opts?.cwd !== undefined) {
  ...
  it('emits status:error and no client.version.changed when a post-install handler throws', async () => {
    const postInstallHandler = mock().mockRejectedValue(new Error('handler exploded'));
    await initManager(makeStrategyDeps(), {
      definition: POST_INSTALL_CLIENT_DEFINITION,
      config: {
        ...managerConfig(),
  ...
  it('client.setActive switches the active pointer to an installed version', async () => {
    ...
    (
      strategyDeps.exec as Mock<(cmd: string, args: string[], opts?: { cwd?: string }) => Promise<string>>
    ).mockImplementation(async (_cmd: string, _args: string[], opts?: { cwd?: string }) => {
      if (opts?.cwd !== undefined) {
        return path.basename(opts.cwd);
      }
      return '';
  ...
  it('client.uninstall still succeeds when removeDirectory throws', async () => {
    const strategyDeps = makeStrategyDeps();
    (strategyDeps.removeDirectory as Mock<(dirPath: string) => Promise<void>>).mockRejectedValue(
