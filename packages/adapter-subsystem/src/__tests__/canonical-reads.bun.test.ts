...
import { type CredentialRef, CredentialRefSchema, buildAccountManagerCredentialRef } from '@makaio/contracts/config';
...
async function createHarness(): Promise<Harness> {
  ...
  const offListProviders = MakaioBus.on(ProviderStorageSubjects.list, (ctx) => {
    ctx.setResult({ providers: [anthropicProvider, openaiProvider] });
  });
...
describe('AdapterSubsystemService canonical reads', () => {
  ...
  it('falls back to the selected provider when provider list discovery fails', async () => {
    const offGetProvider = MakaioBus.on(
      ProviderStorageSubjects.get,
      (ctx) => {
        ctx.setResult({
          provider: {
