...
import { makeStubExtensionContext, waitFor } from '@makaio/test-utils';
 ...
 * @param read - Predicate/read function polled via `waitFor`.
...
describe('SupervisorService', () => {
  ...
  beforeEach(async () => {
    ...
    getLastProcess = mockBackend.getLastProcess;
    getLastSpawnOptions = mockBackend.getLastSpawnOptions;

    const factory: PtyRuntimeFactory = (handlers) => new PtyRuntime(mockBackend.backend, handlers);
...
describe('LazyNodePtyBackend', () => {
  it('single-flights concurrent backend initialization', async () => {
    ...
    const backend: IPtyBackend = {
      spawn: mock().mockResolvedValue(process),
  ...
  it('clears the cached backend when backend dispose rejects', async () => {
    ...
    const firstBackend: IPtyBackend = {
      spawn: mock().mockResolvedValue(firstProcess),
      dispose: mock().mockRejectedValue(disposeError),
    };
    const secondBackend: IPtyBackend = {
      spawn: mock().mockResolvedValue(secondProcess),
      dispose: mock().mockResolvedValue(undefined),
    };
    const createBackend = mock().mockResolvedValueOnce(firstBackend).mockResolvedValueOnce(secondBackend);
    const lazyBackend = new LazyNodePtyBackend(createBackend);
