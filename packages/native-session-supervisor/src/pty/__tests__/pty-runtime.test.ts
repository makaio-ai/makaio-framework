...
function makeRuntime(
  backend: IPtyBackend,
  onOutput: (evt: PtyOutputEvent) => void = vi.fn(),
  onExit: (evt: PtyExitEvent) => void = vi.fn(),
): PtyRuntime {
  ...
  describe('spawn', () => {
    ...
    it('wraps backend errors with context', async () => {
      const backend: IPtyBackend = {
        spawn: vi.fn().mockRejectedValue(new Error('native PTY unavailable')),
      };
      const runtime = makeRuntime(backend);
      runtime.init();
    ...
    it('replaces a pre-existing session with the same supervisorSessionId', async () => {
      ...
      const backend: IPtyBackend = {
        spawn: vi.fn().mockResolvedValueOnce(fakePty1).mockResolvedValueOnce(fakePty2),
      };
      const runtime = makeRuntime(backend);
      runtime.init();
