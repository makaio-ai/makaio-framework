...
import type { RequestContext, SubjectDefinition, TransportReceiveContext } from '@makaio/core';
...
function createBroadcastContext<P, R>(
  ...
  const context: BroadcastContext<P, R> = {
    ...
    // No runtime validation occurs in broadcast — handlers are trusted.
...
>(
  ...
  const localPromises = handlers.map(async (handler) => {
    const handlerContext = createBroadcastContext<Request, Response>(
      ...
      correlationId,
      (nodeId, result) => {
        results.push({ nodeId, payload: result });
      },
      options?.transport,
    );
    ...
    try {
      ...
      console.error(`[${correlationId}][${messageId}] Broadcast handler error for "${subjectKey}":`, error);
    }
  });
