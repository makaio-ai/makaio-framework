...
>(context: MakaioBusContext, subjectDefinition: T, payload: Request, options?: RequestOptions): Promise<Response> {
  ...
  // Validate request payload in development (skip for namespaces with Zod version conflicts)
  const schema = context.namespaceRegistry.getSchema(fullSubjectKey);
  const requestSchema = schema && isRequestSchema(schema) ? schema : undefined;
  const skipValidation = context.namespaceRegistry.shouldSkipValidation(fullSubjectKey);
  if (requestSchema && !skipValidation) {
    validateSchema(subjectKey, payload, requestSchema.request);
  }
  ...
  if (requestSchema && !skipValidation) {
    validateSchema(subjectKey, result, requestSchema.response);
  }
