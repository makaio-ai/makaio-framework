...
describe('Test F: fire-and-forget ctx.next() propagates downstream errors', () => {
  it('rejects with RequestError when handler A fires ctx.next() without await and B throws', async () => {
    ...
    try {
      await expect(bus.request(CursorSubjects.compute, { value: 1 })).rejects.toSatisfy(
        (err: unknown) =>
          err instanceof RequestError && err.cause instanceof Error && err.cause.message === 'downstream',
      );
    } finally {
      bus.disconnect();
      cleanupA();
