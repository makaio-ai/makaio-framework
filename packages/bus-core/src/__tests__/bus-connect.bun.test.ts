...
interface ConnectableTransport {
  ...
  disconnectSpy: ReturnType<typeof mock>;
}

/**
 * Create a minimal transport stub for bus.connect() lifecycle tests.
 *
 * All lifecycle methods are `mock()` spies exposed via the returned
...
function createConnectableTransport(options: ConnectableTransportOptions = {}): ConnectableTransport {
  ...
  const transport: BusTransport = {
    ...
    // Cast required: mock() cannot satisfy the conditional generic overload.
    // Same technique used by createStubTransport in helpers/transport-fixtures.ts.
    send: mock(async (_message: BusMessage) => true) as BusTransport['send'],
    onReceive: mock((_handler: (message: BusMessage) => Promise<void>) => () => {}) as BusTransport['onReceive'],
