 ...
 * via this package's Zod instance. Cross-Zod-version schemas (the skipBusValidation
 ...
 */
export interface NamespaceRegistrationOptions {
  /**
   * Skip Zod schema validation at the bus layer for this namespace.
   *
   * Use this when integrating with libraries that bundle incompatible Zod versions
   * (e.g., Zod v3 vs v4). When enabled, the bus will not validate payloads
   * for subjects in this namespace.
   */
  skipBusValidation?: boolean;
}
 ...
 * and validation-skip flags for namespaces that bundle incompatible Zod versions.
 * @returns Namespace registry with registerNamespace, getSchema, listRegisteredSubjects,
 *   isRequestSubject, isLocalSubject, shouldSkipValidation, and (test-only) __resetNamespaces methods
 */
...
export const createNamespaceRegistry = () => {
  ...
  // Namespaces that should skip validation (for Zod version conflicts)
  const validationExemptNamespaces = new Set<string>();
  ...
  return {
     ...
     * @param options - Registration options (e.g., skipBusValidation)
    ...
    > {
      ...
      // Track validation exemption if requested
      if (options?.skipBusValidation) {
        validationExemptNamespaces.add(domain);
      }
     ...
     * @param subject - Full subject identifier (e.g., "adapter:github-copilot.sdk.event")
     * @returns True if validation should be skipped
     */
    shouldSkipValidation(subject: string): boolean {
      for (const ns of validationExemptNamespaces) {
        if (subject.startsWith(ns + '.')) return true;
      }
      return false;
    },
    /**
    ...
    __resetNamespaces:
      process.env.NODE_ENV === 'test'
        ? () => {
            ...
            localSubjects.clear();
            validationExemptNamespaces.clear();
          }
        : undefined,
  };
};
