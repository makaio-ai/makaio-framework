...
describe('registerDrizzleAdapterSessionStorage', () => {
  ...
  describe('upsert', () => {
    it('should insert new adapter session', async () => {
      ...
      // Verify in DB
      const rows = await ctx.db.run(sql`SELECT * FROM adapter_sessions WHERE adapter_session_id = 'cc-session-1'`);
      expect(rows.rows).toHaveLength(1);
      expect(rows.rows[0].adapter_name).toBe('claude-code');
      expect(rows.rows[0].status).toBe('discovered');
      expect(rows.rows[0].kind).toBe('root');
    });
    ...
    it('should insert with optional fields', async () => {
      ...
      const rows = await ctx.db.run(sql`SELECT * FROM adapter_sessions WHERE adapter_session_id = 'cc-session-2'`);
      expect(rows.rows).toHaveLength(1);
      expect(rows.rows[0].parent_adapter_session_id).toBe('cc-parent-1');
      expect(rows.rows[0].fork_point_message_id).toBe('msg-fork-1');
      expect(rows.rows[0].kind).toBe('fork');
      expect(rows.rows[0].model).toBe('claude-3-opus');
      expect(rows.rows[0].cwd).toBe('/home/user/project');
    });
    ...
    it('should update existing adapter session on upsert', async () => {
      ...
      const rows = await ctx.db.run(sql`SELECT * FROM adapter_sessions WHERE adapter_session_id = 'cc-session-3'`);
      expect(rows.rows).toHaveLength(1);
      expect(rows.rows[0].parent_adapter_session_id).toBe('cc-parent-new');
      expect(rows.rows[0].fork_point_message_id).toBe('msg-fork-new');
      expect(rows.rows[0].kind).toBe('fork');
      expect(rows.rows[0].model).toBe('claude-3-opus');
      expect(rows.rows[0].cwd).toBe('/new/path');
    });
    ...
    it('should preserve existing model/cwd if not provided in update', async () => {
      ...
      const rows = await ctx.db.run(sql`SELECT * FROM adapter_sessions WHERE adapter_session_id = 'cc-session-4'`);
      expect(rows.rows[0].model).toBe('claude-3-sonnet');
      expect(rows.rows[0].cwd).toBe('/original/path');
    });
    ...
    it('should store an explicit startedAt value on insert', async () => {
      ...
      expect(rows.rows).toHaveLength(1);
      expect(rows.rows[0].started_at).toBe(startedAt);
    });
    ...
    it('should not overwrite startedAt on re-upsert (write-once semantics)', async () => {
      ...
      expect(rows.rows[0].started_at).toBe(startedAtT1);
    });
    ...
    it('should backfill startedAt when the existing value is the auto-discovery placeholder', async () => {
      ...
      const beforeRows = await ctx.db.run(
      ...
      expect(beforeRows.rows[0].started_at).toBe(beforeRows.rows[0].discovered_at);
      ...
      const afterRows = await ctx.db.run(
      ...
      expect(afterRows.rows[0].started_at).toBe(canonicalStartedAt);
    });
    ...
    it('should default startedAt to approximately Date.now() when not provided', async () => {
      ...
      expect(rows.rows).toHaveLength(1);
      const storedStartedAt = rows.rows[0].started_at as number;
