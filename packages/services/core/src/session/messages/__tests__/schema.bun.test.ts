...
describe('messages schema', () => {
  ...
  it('should allow null turnId for native imports', async () => {
    ...
    const rows = await db.all<{ turn_id: string | null }>(sql`SELECT * FROM messages WHERE message_id = 'msg-1'`);
  ...
  it('should still support turnId when provided', async () => {
    ...
    const rows = await db.all<{ turn_id: string | null }>(sql`SELECT * FROM messages WHERE message_id = 'msg-2'`);
    expect(rows).toHaveLength(1);
  ...
  it('should cascade delete messages when turn is deleted', async () => {
    ...
    const rows = await db.all(sql`SELECT * FROM messages WHERE message_id = 'msg-3'`);
  ...
  it('should store adapterMessageId for CC message tracking', async () => {
    ...
    const rows = await db.all<{ adapter_message_id: string | null }>(
