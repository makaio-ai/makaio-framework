...
import { pgTable, text, integer, uniqueIndex, index, primaryKey, type AnyPgColumn } from 'drizzle-orm/pg-core';
import { epochMs, bool, jsonCol, float8 } from '@makaio/storage-drizzle/columns/postgres';
import type {
  WorkflowDefinition,
 ...
 */
export const workflowExecutionFrames = pgTable(
  'workflow_execution_frames',
  {
    ...
    executionId: text('execution_id')
      .notNull()
      .references((): AnyPgColumn => workflowExecutions.id, { onDelete: 'cascade' }),
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const workflowGateInstances = pgTable(
 ...
 */
export const workflowStepSpans = pgTable(
  'workflow_step_spans',
  {
 ...
 */
export const workflowExecutionLinks = pgTable(
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const workflowRunContexts = pgTable(
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const worklogSummaries = pgTable(
  'worklog_summaries',
  {
    /** Unique execution identifier (1:1 with workflow_executions.id). */
    executionId: text('execution_id')
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const worklogFrameEntries = pgTable(
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const worklogArtifactWrites = pgTable(
 ...
 * CENTRAL tier — migrated alongside `workflow_executions`.
 */
export const worklogGateEvents = pgTable(
  'worklog_gate_events',
  {
