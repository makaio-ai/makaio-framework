...
/**
 * Dispatch a definition-backed execution through Worker requirements, the
...
export function launchDefinitionExecutionTask(
  ...
  if (deps.executionAttemptAuthority !== undefined) {
    const workerRunner = createWorkerDispatchRunner({
      bus: deps.bus,
    ...
    if (workerRunner !== undefined) {
      return buildExecutionTask(deps.buildRunnerTaskDeps(workerRunner), {
        ...params,
