 ...
 *   (framework-only boot)
 */
export async function resolveClientBinary(clientId: string): Promise<ClientExecutionContext | undefined> {
  const result = await MakaioBus.requestOptional(ClientSubjects.resolveBinary, { clientId });
