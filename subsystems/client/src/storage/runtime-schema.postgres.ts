/**
 * Postgres twin for the `client_runtimes` table.
 *
 * Column semantics are identical to the canonical SQLite schema. Timestamp
 * columns use `bigint` in `'number'` mode (int53-safe epoch milliseconds);
 * JSON columns use `jsonb`; OS PID columns stay `integer` (int4 — PID space
 * fits comfortably within 32-bit range on every supported platform).
 * @packageDocumentation
 */

import { index, integer, pgTable, text } from 'drizzle-orm/pg-core';
import { epochMs, jsonCol } from '@makaio/storage-drizzle/columns/postgres';
 ...
 */
export const clientRuntimes = pgTable(
  'client_runtimes',
  {
