...
describe.skipIf(!RUNTIME_ENABLED)('runtime lifecycle integration', () => {
  ...
  it('startup() completes without error', async () => {
    const { startup } = await import('../../src/runtime/index.js');

    await expect(startup()).resolves.toBeUndefined();
  });

  it('startup() is idempotent — second call is a no-op', async () => {
    const { startup } = await import('../../src/runtime/index.js');
  ...
  it('ensureRuntime() returns the MakaioBus singleton', async () => {
  ...
  it('shutdown() cleans up without error after a successful boot', async () => {
  ...
  it('shutdown() is a no-op when the runtime was never started', async () => {
    const { shutdown } = await import('../../src/runtime/index.js');
  ...
  it('re-boots cleanly after a prior shutdown', async () => {
