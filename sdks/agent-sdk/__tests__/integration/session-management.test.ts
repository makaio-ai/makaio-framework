...
describe.skipIf(!RUNTIME_ENABLED)('session management — live runtime (requires MAKAIO_TEST_RUNTIME)', () => {
  ...
  it('listSessions() returns at least one session after a query', { timeout: 120_000 }, async () => {
  ...
  it('getSessionInfo() returns the session created by a prior query', { timeout: 120_000 }, async () => {
  ...
  it('renameSession() updates the session title', { timeout: 120_000 }, async () => {
    const {
      startup,
      query,
  ...
  it('deleteSession() removes the session from the list', { timeout: 120_000 }, async () => {
