...
// ---------------------------------------------------------------------------
// Hoisted mock state — must be declared with vi.hoisted() so it is available
// inside the vi.mock() factory, which vitest lifts above all imports.
// ---------------------------------------------------------------------------

const { connectMock, closeMock, getBusMock } = vi.hoisted(() => ({
  connectMock: vi.fn<() => Promise<void>>(() => Promise.resolve()),
  closeMock: vi.fn<() => void>(),
  getBusMock: vi.fn<() => object>(() => ({})),
}));
...
describe('ensureConnection — singleton caching', () => {
  ...
  it('returns the IMakaioBus from getBus()', async () => {
    const fakeBus = { on: vi.fn(), request: vi.fn(), emit: vi.fn() };
