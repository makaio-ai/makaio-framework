...
// ---------------------------------------------------------------------------
// Mock ensureConnection to return the shared MakaioBus singleton
// ---------------------------------------------------------------------------
...
describe('startup()', () => {
  ...
  it('passes websocket options through to ensureConnection', async () => {
    ...
    expect(connectionMock.ensureConnection).toHaveBeenCalledWith({
...
describe('query()', () => {
  ...
  it('dispatches async iterable follow-up messages to the same session', async () => {
    ...
    await vi.waitFor(() => expect(sendMessagePayloads).toHaveLength(2));
