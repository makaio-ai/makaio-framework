...
| `clientId` | `string` | yes |
| `lastClientIdentityObservation` | `{ clientId: string; source: string; kind: string; observedAt: number; payload: unknown; }` | yes |
...
| `status` | `"active" \| "closed" \| "archived" \| "discovered" \| "all" \| undefined` | no |
