...
function createMockSettings(
  ...
  return {
    listHooks: mock().mockResolvedValue({
      effective: existingHooks,
      perScope: [],
    }),
    addHook: mock().mockResolvedValue({ added: true }),
    removeHook: mock().mockResolvedValue({ removed: 1 }),
    listStatusline: mock().mockResolvedValue({
      effective: effectiveStatusline,
      perScope: statuslinePerScope,
    }),
    setStatusline: mock().mockResolvedValue({
      previous: null,
      applied: { type: 'command', command: 'makaio claude statusline' },
    }),
    removeStatusline: mock().mockResolvedValue({ previous: null, removed: false }),
    setSkipDangerousModePermissionPrompt: mock().mockResolvedValue(undefined),
  };
}
...
describe('applyClaudeCodeWiring', () => {
  ...
  it('counts the statusline as applied when setStatusline previous was null', async () => {
    (settings.setStatusline as ReturnType<typeof mock>).mockResolvedValue({
      previous: null,
      applied: { type: 'command', command: 'makaio claude statusline' },
    });
    const result = await applyClaudeCodeWiring(settings, 'user', 'makaio');
  ...
  it('counts skipped when addHook returns added=false', async () => {
    (settings.addHook as ReturnType<typeof mock>).mockResolvedValue({ added: false });
  ...
  it('passes the correct scope to addHook', async () => {
    ...
    const calls = (settings.addHook as ReturnType<typeof mock>).mock.calls as [{ scope: string }][];
  ...
  it('passes the correct scope to setStatusline', async () => {
    ...
    const call = (settings.setStatusline as ReturnType<typeof mock>).mock.calls[0] as [{ scope: string }];
  ...
  it('passes matcher empty string to addHook for catch-all matching', async () => {
    await applyClaudeCodeWiring(settings, 'user', 'makaio');
    const calls = (settings.addHook as ReturnType<typeof mock>).mock.calls as [{ matcher?: string }][];
  ...
  it('embeds existing statusline command as upstream sh renderer when setting Makaio statusline', async () => {
    ...
    const settingsWithExisting: ClaudeCodeWiringSettings = {
      listHooks: mock().mockResolvedValue({ effective: {}, perScope: [] }),
      addHook: mock().mockResolvedValue({ added: true }),
      removeHook: mock().mockResolvedValue({ removed: 1 }),
      listStatusline: mock().mockResolvedValue({
    ...
    const setCall = (settingsWithExisting.setStatusline as ReturnType<typeof mock>).mock.calls[0] as [
  ...
  it('does not add upstream renderer args when no previous statusline exists', async () => {
    ...
    await applyClaudeCodeWiring(settings, 'user', 'makaio');
    const setCall = (settings.setStatusline as ReturnType<typeof mock>).mock.calls[0] as [
      { value: { command: string } },
    ];
  ...
  it('does not add upstream renderer args when previous statusline is already a Makaio command', async () => {
    ...
    const setCall = (settingsWithMakaio.setStatusline as ReturnType<typeof mock>).mock.calls[0] as [
      { value: { command: string } },
    ];
    expect(setCall[0].value.command).not.toContain('--upstream-command');
  });
  ...
  it('writes hook commands containing the sentinel and event name', async () => {
    await applyClaudeCodeWiring(settings, 'user', 'makaio');
    const calls = (settings.addHook as ReturnType<typeof mock>).mock.calls as [{ hook: { command: string } }][];
  ...
  it('prepends envPairs before the executable in hook and statusline commands', async () => {
    ...
    const hookCalls = (settings.addHook as ReturnType<typeof mock>).mock.calls as [{ hook: { command: string } }][];
    for (const [req] of hookCalls) {
      expect(req.hook.command).toMatch(/^MAKAIO_CONFIG_FILE=.+ MAKAIO_HOME=.+ \/path\/to\/cli-entry\.ts /);
    }

    const statuslineCalls = (settings.setStatusline as ReturnType<typeof mock>).mock.calls as [
      { value: { command: string } },
    ][];
  ...
  it('removes stale hook before adding when sentinel matches but makaioCommand prefix differs (replace semantics)', async () => {
    ...
    const staleSettings: ClaudeCodeWiringSettings = {
      ...
      addHook: mock().mockImplementation(async (req: { eventName: string }) => {
        callLog.push({ op: 'add', eventName: req.eventName });
        return { added: true };
      }),
      removeHook: mock().mockImplementation(async (req: { eventName: string }) => {
        callLog.push({ op: 'remove', eventName: req.eventName });
        return { removed: 1 };
      }),
      listStatusline: mock().mockResolvedValue({ effective: null, perScope: [] }),
      setStatusline: mock().mockResolvedValue({
        previous: null,
        applied: { type: 'command', command: 'makaio claude statusline' },
      }),
    ...
    const removeCalls = (staleSettings.removeHook as ReturnType<typeof mock>).mock.calls as [
    ...
    const addCalls = (staleSettings.addHook as ReturnType<typeof mock>).mock.calls as [
...
describe('removeClaudeCodeWiring', () => {
  ...
  it('calls removeStatusline when the target scope has a Makaio statusline without upstream', async () => {
    ...
    (settingsWithStatusline.removeStatusline as ReturnType<typeof mock>).mockResolvedValue({
      previous: { type: 'command', command: 'makaio claude statusline' },
      removed: true,
    });
  ...
  it('restores the original upstream command when unwiring a statusline with --upstream', async () => {
    ...
    (settingsWithUpstream.setStatusline as ReturnType<typeof mock>).mockResolvedValue({
      previous: { type: 'command', command: makaioCommand },
  ...
  it('preserves extra statusline fields like padding when restoring upstream', async () => {
    ...
    (settingsWithPadding.setStatusline as ReturnType<typeof mock>).mockResolvedValue({
    ...
    const setCall = (settingsWithPadding.setStatusline as ReturnType<typeof mock>).mock.calls[0] as [
  ...
  it('restores the upstream command when trailing flags follow the JSON array', async () => {
    ...
    (settingsWithTrailing.setStatusline as ReturnType<typeof mock>).mockResolvedValue({
      previous: { type: 'command', command: makaioCommand },
      applied: { type: 'command', command: 'npx -y ccstatusline@latest' },
    });
  ...
  it('removes user scope Makaio statusline even when project scope overrides with a non-Makaio entry', async () => {
    ...
    (settingsWithProjectOverride.removeStatusline as ReturnType<typeof mock>).mockResolvedValue({
      previous: { type: 'command', command: 'makaio claude statusline' },
      removed: true,
    });
  ...
  it('passes the correct scope to removeHook', async () => {
    ...
    const calls = (settings.removeHook as ReturnType<typeof mock>).mock.calls as [{ scope: string }][];
    expect(calls.every((args) => args[0].scope === 'project')).toBe(true);
  });

  it('returns removed=0 when all removeHook calls return 0 and no statusline', async () => {
    (settings.removeHook as ReturnType<typeof mock>).mockResolvedValue({ removed: 0 });
    const result = await removeClaudeCodeWiring(settings, 'user');
