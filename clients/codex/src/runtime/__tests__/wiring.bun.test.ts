...
/**
 * Version of {@link CodexWiringSettings} where each method is a typed
 * `Mock<T>` so tests can access `.mock.calls` and `mockResolvedValue` without
 * unsafe `as any` casts.
 */
type MockCodexWiringSettings = {
  [K in keyof CodexWiringSettings]: Mock<CodexWiringSettings[K]>;
};
 ...
 */
function createMockSettings(effectiveHooks: CodexHookEntry[] = []): MockCodexWiringSettings {
  return {
    listHooks: mock<CodexWiringSettings['listHooks']>().mockResolvedValue({
      effective: effectiveHooks,
      perScope: [],
    }),
    addHook: mock<CodexWiringSettings['addHook']>().mockResolvedValue({ added: true }),
    removeHook: mock<CodexWiringSettings['removeHook']>().mockResolvedValue({ removed: 1 }),
  };
}
...
describe('applyCodexWiring', () => {
  ...
  it('passes the correct scope to addHook', async () => {
    ...
    const calls = settings.addHook.mock.calls;
  ...
  it('skips already-installed entries and reflects the count', async () => {
    ...
    settings.addHook.mockResolvedValue({ added: false });
  ...
  it('removes the stale hook before adding the updated one when the binary prefix changes', async () => {
    ...
    const settings: CodexWiringSettings = {
      ...
      addHook: mock().mockImplementation(async (req: { event: string }) => {
        callLog.push({ op: 'add', event: req.event });
        return { added: true };
      }),
      removeHook: mock().mockImplementation(async (req: { event: string }) => {
...
describe('removeCodexWiring', () => {
  ...
  it('passes the correct scope to removeHook', async () => {
    ...
    const calls = settings.removeHook.mock.calls;
    expect(calls.every(([req]) => req.scope === 'project')).toBe(true);
  });
