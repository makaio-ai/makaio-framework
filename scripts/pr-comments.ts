 ...
 * tsx scripts/pr-comments.ts https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Only feedback from the last 10 minutes
 * tsx scripts/pr-comments.ts --since 10 https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Only print entries not seen in the local state file, then record them as seen
 * tsx scripts/pr-comments.ts --new https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Poll every 30 seconds for up to 20 minutes until new actionable entries appear
 * tsx scripts/pr-comments.ts --new --timeout 20 https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Use a custom state file
 * tsx scripts/pr-comments.ts --new --state-file .tmp/my-pr-comment-state.json https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Include resolved thread comments
 * tsx scripts/pr-comments.ts --resolved https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Include outdated thread comments
 * tsx scripts/pr-comments.ts --outdated https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Keep review walkthrough and auto-generated markup instead of stripping it
 * tsx scripts/pr-comments.ts --raw https://github.com/makaio-ai/makaio-framework/pull/86
 *
 * # Stream new findings continuously (implies --new, runs until SIGTERM)
 * tsx scripts/pr-comments.ts --watch https://github.com/makaio-ai/makaio-framework/pull/86
 * ```
 */
...
/** Host-facing command printed by the generic renderer for the next review pass. */
const NEXT_POLL_COMMAND = 'tsx scripts/pr-comments.ts --new --timeout 20 <pr-url>';
 ...
 *
 * Linked entrypoints can invoke this script through a symlink, so both sides
 * of the comparison must be resolved to real paths before comparing.
...
if (isMain) {
  main().catch((error: Error) => {
    console.error(process.env.DEBUG ? (error.stack ?? error.message) : error.message);
    process.exit(1);
  });
}
