...
export function extractSubjects(
  ...
  if (hit.schemaRecordName === '<inline>') {
    if (hit.inlineSchemaNode) {
      return extractSubjectsFromObjectLiteral(hit.prefix, hit.inlineSchemaNode, checker, hit.filePath, analysisRoot);
    }
    return extractSubjectsFromInlineArg(hit, sourceFile, checker, analysisRoot);
  }
  ...
  if (exportedSymbol) return extractSubjectsFromSymbol(hit.prefix, exportedSymbol, checker, hit.filePath, analysisRoot);

  // 2. Imported symbol (e.g. `import { FooSchemas } from './schemas.js'`)
  const imported = extractSubjectsFromImport(hit, sourceFile, program, checker, analysisRoot);
  ...
  if (localSymbol) return extractSubjectsFromSymbol(hit.prefix, localSymbol, checker, hit.filePath, analysisRoot);
  ...
  for (const stmt of namespaceFile.statements) {
    ...
    for (const element of stmt.importClause.namedBindings.elements) {
      ...
      if (symbol) {
        ...
        return extractSubjectsFromSymbol(hit.prefix, aliased, checker, hit.filePath, analysisRoot);
      }
    }
  }
 ...
 * @returns Sorted array of field descriptors, or `undefined` when `_output` is absent.
 */
function extractFieldsFromZodType(checker: ts.TypeChecker, zodType: ts.Type): SubjectField[] | undefined {
  ...
  for (const field of outputType.getProperties()) {
    ...
    const typeString = checker.typeToString(fieldType, undefined, ts.TypeFormatFlags.NoTruncation);
...
function extractSymbolMetadata(
  ...
  const docs = originSymbol.getDocumentationComment(checker);
  if (docs.length > 0) {
    const text = docs
      .map((d) => d.text)
      .join('')
...
function buildSubjectEntry(
  ...
  if (isRpc) {
    ...
    return {
      ...
      schemaFile,
      description,
      request: extractFieldsFromZodType(checker, requestType),
      response: extractFieldsFromZodType(checker, responseType),
    };
  }
  ...
  return {
    ...
    schemaFile,
    description,
    payload: extractFieldsFromZodType(checker, baseType),
  };
}
...
function extractSubjectsFromSymbol(
  ...
  for (const prop of type.getProperties()) {
    ...
    subjects.push(buildSubjectEntry(key, prefix, baseType, checker, prop, namespaceFilePath, analysisRoot));
  }
  ...
  for (const stmt of sourceFile.statements) {
    ...
    for (const decl of stmt.declarationList.declarations) {
      ...
      if (schemasArg && ts.isObjectLiteralExpression(schemasArg)) {
        return extractSubjectsFromObjectLiteral(hit.prefix, schemasArg, checker, hit.filePath, analysisRoot);
      }
    }
  }
