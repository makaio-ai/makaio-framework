...
/**
 * Check whether a candidate belongs to a more specific shared subpath.
 * @param candidate - Manifest key, chunk path, or module ID to inspect.
 * @param dep - Shared external currently being resolved.
 * @returns True when the candidate should be reserved for a longer shared external.
 */
function matchesMoreSpecificSharedExternal(candidate: string, dep: SharedBrowserExternal): boolean {
  for (const otherDep of SHARED_BROWSER_EXTERNALS) {
    if (otherDep === dep || !otherDep.startsWith(`${dep}/`)) continue;
    if (matchesDep(candidate, otherDep) || matchesSharedEntry(candidate, toSharedBrowserExternalEntryName(otherDep))) {
      return true;
    }
  }
  return false;
...
export function generateImportMap(manifestPath: string, outputPath: string, basePath = './'): void {
  ...
  for (const dep of SHARED_BROWSER_EXTERNALS) {
    ...
    for (const [key, entry] of Object.entries(manifest)) {
      const candidates = [key, ...(entry.src === undefined ? [] : [entry.src])];
      if (
        candidates.some((candidate) => matchesDep(candidate, dep)) &&
        candidates.every((candidate) => !matchesMoreSpecificSharedExternal(candidate, dep))
      ) {
  ...
  for (const dep of SHARED_BROWSER_EXTERNALS) {
    ...
    for (const chunk of Object.values(bundle)) {
      ...
      const match = candidates.reduce<string | undefined>((shortestMatch, candidate) => {
        ...
        if (matchesMoreSpecificSharedExternal(candidate, dep)) {
          return shortestMatch;
        }
