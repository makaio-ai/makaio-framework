...
const THEME_DIR = path.resolve(import.meta.dirname, '..');
...
describe('style rules', () => {
  it('ensures theme CSS vars referenced in modules exist in aura theme', async () => {
    ...
    const scssFiles = (await globby(['**/*.module.scss'], { cwd: UI_ROOT, absolute: true })).filter(
      (file) => !shouldIgnoreFile(path.relative(UI_ROOT, file)),
    );
  ...
  it('rejects literal rgba/hex colors outside theme packages', async () => {
    const scssFiles = (await globby(['**/*.scss'], { cwd: UI_ROOT, absolute: true })).filter(
      (file) => !shouldIgnoreFile(path.relative(UI_ROOT, file)),
    );
