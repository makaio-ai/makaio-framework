...
describe('registerExtensionUI', () => {
  ...
  it('rolls back earlier registrations when a later registration throws', () => {
    const bus = createBusInstance();
    const pageDefinitionRegisterSpy = vi.spyOn(pageDefinitionRegistry, 'register').mockImplementation(() => {
  ...
  it('runs rollback cleanups in reverse order and suppresses cleanup failures', () => {
    ...
    const pageRegisterSpy = vi.spyOn(pageRegistry, 'register').mockImplementation(() => {
      return () => {
    ...
    const registerWidgetSpy = vi.spyOn(widgetRegisterModule, 'registerWidget').mockImplementation(() => {
