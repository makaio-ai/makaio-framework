 ...
 */
export interface CleanEnvForAdapterOptions {
  /**
 ...
 * ```
 */
export function cleanEnvForAdapter(env?: AdapterEnv, options: CleanEnvForAdapterOptions = {}): Record<string, string> {
  ...
  for (const key of options.omitEnvVars ?? []) {
    delete result[key];
  }
  return result;
}
