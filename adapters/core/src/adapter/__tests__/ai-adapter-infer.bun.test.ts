...
import { createMockScopedBus, advanceTimersByTimeAsync } from '@makaio/test-utils';
...
describe('AIAdapter.handleInfer', () => {
  ...
  it('ephemeral agent: evicts when the initial turn never completes', async () => {
    ...
    try {
      ...
      await advanceTimersByTimeAsync(EPHEMERAL_CLEANUP_COMPLETION_TIMEOUT_MS);
      // Drain the multi-level async chain: waitForCompletion → throw → finally evictSilently
      // → agent.close → catch → console.warn. Each await hop needs a microtask flush.
      for (let i = 0; i < 8; i++) {
        await Promise.resolve();
      }
