...
| `chatInTurnMessages` | `caps.chatInTurnMessages` | Current runtime token for multiple user messages in one turn |
| `chat:inTurnMessages` | `caps.chatInTurnMessages` | Registry-shaped spelling for the same concept; use only when the declaring adapter also emits this token |
...
the adapter. `chatInTurnMessages` is the compatibility token used by current adapters; do not
