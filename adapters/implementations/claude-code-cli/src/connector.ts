...
import { resolveConnectorCredentials, buildCredentialEnv } from '@makaio/ai-adapters-core/config';
 ...
 * @returns SDK payload without connector-managed metadata fields.
 */
function stripSdkMetadata<T extends Record<string, unknown>>(payload: T): Omit<T, (typeof SDK_METADATA_KEYS)[number]> {
  const sanitized: Record<string, unknown> = { ...payload };
  for (const key of SDK_METADATA_KEYS) {
    delete sanitized[key];
  }
  return sanitized as Omit<T, (typeof SDK_METADATA_KEYS)[number]>;
}
...
export class ClaudeCliConnector extends AIAgentConnector<ClaudeCodeCliConnectorBus> {
  ...
  private async initializeSession(): Promise<void> {
    ...
    const providerContext = this.config.providerContext;
    const credentialRefs = providerContext?.credentialRefs ?? {};
    const creds = await resolveConnectorCredentials(this.config.bus, credentialRefs);
    const credEnv = buildCredentialEnv(creds, providerContext?.credentialEnvVars);

    // Resolve the binary for this client. Uses requestOptional so the adapter
    // continues to function in framework-only boot where no handler is registered.
    // The resolved env is merged AFTER credential env so binary-level overrides
    // (e.g. CLAUDE_CONFIG_DIR) take precedence over credential values.
    const resolvedBinary = await resolveClientBinary('claude-code');

    // Resolved credential env must win over connector defaults so rotated
    // secrets are not masked by stale process-level values.
    // Resolved binary env then takes precedence to enforce config isolation.
    const sessionEnv = { ...this.env, ...credEnv, ...(resolvedBinary?.env ?? {}) };

    // Explicit providerConfig.binaryPath is a user override and always wins.
    // Falls back to the resolved binary path, then to PATH lookup (undefined).
    const binaryPath =
    ...
    const sessionConfig: ClaudeCliSessionConfig = {
      ...
      emitSdkEvent: async (msg) => {
        ...
        await this.emit(ClaudeCodeCliConnectorSubjects.sdk.event, stripSdkMetadata(msg));
      },
      onTurnStart: (handle) => {
