...
describe('createStdioTransport', () => {
  ...
  it('emits error and kills process when firstOutputTimeoutMs elapses with no stdout', () => {
    vi.useFakeTimers();
  ...
  it('clears firstOutputTimeout when stdout data arrives before deadline', () => {
    vi.useFakeTimers();
    const transport = createStdioTransport(['--print'], '/tmp', {}, undefined, 5000);
    const errors: Error[] = [];
    transport.onError((error) => errors.push(error));
  ...
  it('clears firstOutputTimeout on explicit close() without emitting error', () => {
    vi.useFakeTimers();
    const transport = createStdioTransport(['--print'], '/tmp', {}, undefined, 5000);
    const errors: Error[] = [];
    transport.onError((error) => errors.push(error));
  ...
  it('merges process env and strips CLAUDECODE from spawn env', () => {
    ...
    try {
      ...
      createStdioTransport(['--print'], '/tmp', { CUSTOM_VAR: 'value' });
