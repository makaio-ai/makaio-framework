import { KNOWN_SDK_MESSAGE_TYPES, KNOWN_SYSTEM_SUBTYPES, type SDKMessage } from '@makaio/client-claude-code';
...
export class ClaudeCliSession extends BaseConnectorSession<ClaudeCliSessionConfig> {
  ...
  public async startTurn(handle: MessageHandle, mergedContent?: string[]): Promise<void> {
    ...
    transport.onMessage((msg) => {
      // Ignore if a newer subprocess has replaced this transport
      if (this.transport !== transport) return;
      // Skip unknown message types from newer CLI versions (e.g., rate_limit_event)
      if (!KNOWN_SDK_MESSAGE_TYPES.has(msg.type)) return;
      // Skip unknown system subtypes (e.g., hook_started, status)
      if (msg.type === 'system' && !KNOWN_SYSTEM_SUBTYPES.has((msg as { subtype?: string }).subtype ?? '')) return;
   ...
   */
  private async handleSdkMessage(msg: SDKMessage, handle: MessageHandle): Promise<void> {
    ...
    // Emit event through connector (adds metadata like adapterSessionId)
    if (this.emitSdkEvent) {
      await this.emitSdkEvent(msg);
    } else {
      await this.bus.emit(ClaudeCodeCliConnectorSubjects.sdk.event, msg);
    }

    // Handle result message — complete the turn
    if (msg.type === 'result' && this.currentTurn) {
      const isSuccess = msg.subtype === 'success' && !msg.is_error;
      const result: MessageResult = isSuccess
        ? {
            outcome: 'completed',
            result: { message: 'result' in msg && typeof msg.result === 'string' ? msg.result : '' },
          }
        : {
            outcome: 'error',
            error: new Error(msg.subtype ?? 'Unknown CLI error'),
          };
    ...
    if (this.currentTurn) {
      await this.currentTurn.handleSdkEvent(msg);
    }
  }
