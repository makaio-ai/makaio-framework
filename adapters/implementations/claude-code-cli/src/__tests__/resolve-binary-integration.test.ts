 ...
 *
 * Verifies that `initializeSession()` correctly threads the resolved binary
 * context into the session config so the CLI subprocess is spawned with the
 * right binary path and environment.
 *
 ...
 */
async function makeConnector(opts: { binaryPath?: string } = {}): Promise<ClaudeCliConnector> {
