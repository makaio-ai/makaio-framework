...
// ---------------------------------------------------------------------------
// Session harness state — shared mutable state used by the mock class and tests
// ---------------------------------------------------------------------------
...
function resetSessionHarness(): void {
  currentConfig = undefined;
  currentQueryInstance = undefined;
  currentAdapterSessionId = 'adapter-session-1';
}

function fireTurnStartHarness(handle: MessageHandle): void {
...
function fireTurnCompleteHarness(handle: MessageHandle, result: MessageResult): void {
...
mock.module('../src/session.js', () => ({
  ClaudeConnectorSession: MockClaudeConnectorSession,
}));
...
async function fireTurnComplete(
  ...
  fireTurnStartHarness(handle);
  fireTurnCompleteHarness(handle, result);
  return handle;
}

// Real session-path coverage for the onTurnComplete seam lives in
// `session-on-turn-complete.bun.test.ts`; this suite focuses on connector-side
...
describe('ClaudeSdkConnector account observation emission', () => {
  ...
  it('emits through the onTurnComplete session seam after successful completed turns', async () => {
    ...
    const accountInfo = mock<() => Promise<Record<string, unknown>>>()
    ...
    currentQueryInstance = { accountInfo };
  ...
  it('does not emit when the turn result is not completed', async () => {
    const connector = await makeConnector();
    const accountInfo = mock<() => Promise<Record<string, unknown>>>().mockResolvedValue({
      email: 'USER@example.com',
      organization: 'Org One',
      subscriptionType: 'Claude Team',
      apiProvider: 'firstParty',
  ...
  it('keeps turn completion best-effort when client.session.account.observe throws', async () => {
    ...
    const requestOptionalSpy = spyOn(MakaioBus, 'requestOptional').mockRejectedValueOnce(
      new Error('account observation failed'),
    );

    currentQueryInstance = {
      accountInfo: mock().mockResolvedValue({
