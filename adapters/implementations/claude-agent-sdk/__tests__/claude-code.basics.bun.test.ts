...
describe.todo('Claude Code Basics', () => {
  ...
  beforeAll(async () => {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    const secondCallHandle = await connector.sendMessage(normalizeMessageInput('Remind me: What was my first prompt?'));
  ...
  describe('Session Handling', () => {
    ...
    it('uses consistent sessionId for all events within a turn', () => {
      ...
      for (const event of firstTurnEvents) {
        expect(event.payload.session_id).toBe(firstTurnSessionId);
      }
    });
    ...
    it('getAdapterSessionId() matches event session IDs for second turn', () => {
      ...
      for (const event of secondTurnEvents) {
        expect(event.payload.session_id).toBe(secondTurnSessionId);
      }
    });
