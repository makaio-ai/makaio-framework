...
if (!fs.existsSync(logPath)) {
  try {
    ...
    // sometimes due to parallelism folder was created in parallel process
  }
}
...
export type ImportedAdapter = {
  createTestConfig: () => Promise<ConformanceTestConfig>;
};
/**
...
export const getAgentTestContext = async (
  ...
  const testConfig = await adapterImport.createTestConfig();
