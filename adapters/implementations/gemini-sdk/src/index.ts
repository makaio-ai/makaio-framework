 ...
 * Gemini SDK Adapter
 *
 * Package: `@makaio/ai-adapters-gemini-sdk`
 *
...
import { providerDefinition as testProviderDef } from '@makaio/provider-google';
...
export const createTestConfig = async (): Promise<
  ConformanceTestConfig<GeminiConnectorBus, GeminiConnector, GeminiAgent>
> => {
  const { scopedBus } = GeminiConnectorNamespace;
  const bus = await scopedBus();

  if (!testProviderDef.defaultModel) {
    throw new Error(`[gemini-sdk] Invalid test provider definition '${testProviderDef.id}': missing defaultModel`);
  }
  const primaryModelName = testProviderDef.fastModel ?? testProviderDef.defaultModel;

  return {
    createConnector: async (options) =>
      new GeminiConnector(await GeminiSdkConfig.getConfig(resolveTestConfig(options, bus, testProviderDef))),
    bus,
    registerToolApprovalHandler,
    capabilities: {
