 ...
 * for diagnosis and caller-controlled release. No Git acquisition or digest checks.
...
export async function bindLocalWorkspace(options: LocalWorkspaceOptions): Promise<LocalWorkspaceHandle> {
  ...
  if (requirement.sourceRoots.some((source) => source.source !== undefined)) {
    throw new Error('Source acquisition is not implemented');
  }
  ...
  for (const source of requirement.sourceRoots) {
    sourceRoots.push({
      id: source.id,
      path: await resolveSourceRoot(canonicalRoot, source.path, requirement.provisioning === 'create'),
    });
  }
