...
import type { FileConfigStorage } from './file-config-storage.js';
 ...
 * @param storage - File-backed config storage.
 * @returns Cleanup function for registered handlers.
 */
export function registerConfigHandlers(bus: IMakaioBus, storage: FileConfigStorage): () => void {
  const cleanups = [
