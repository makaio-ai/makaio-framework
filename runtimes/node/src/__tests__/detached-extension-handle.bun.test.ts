...
const createProcessLifecycleMock = mock();

mock.module('@makaio/subprocess', () => ({
  createProcessLifecycle: createProcessLifecycleMock,
}));
...
describe('createDetachedExtensionPackage', () => {
  beforeEach(() => {
    createProcessLifecycleMock.mockReset();
  });
  ...
  describe('create() factory', () => {
    ...
    it('requires a host bus URL before starting bus-websocket children', async () => {
      ...
      expect(createProcessLifecycleMock).not.toHaveBeenCalled();
    });
    ...
    it('injects bus-websocket connection environment and runs lifecycle init', async () => {
      ...
      createProcessLifecycleMock.mockReturnValue({
        start,
        stop,
        state: 'idle',
        transport: undefined,
      ...
      const registerNamespace = spyOn(bus, 'registerNamespace');
      const request = spyOn(bus, 'request').mockResolvedValue({ ready: true } as never);
      ...
      expect(createProcessLifecycleMock).toHaveBeenCalledWith(
