...
export class NodeRuntimeProvider extends ConfigProvider {
  ...
  public override async getConfig(overrides?: Partial<Config>): Promise<Config> {
    ...
    // Auto-generate a secret for the effective runtime config when a remote
    // bus URL exists without one. Persist only when the remote bus URL came
    // from stored config; env-only bus URLs must not be written to config.json.
    if (config.bus?.remote && !config.bus.remote.secret) {
      ...
      if (storedConfig.bus?.remote?.url === config.bus.remote.url) {
        await this.saveConfig(await this.prepareConfigForPersistence(config));
      }
    }
